...
Откройте вторую SSH-сессию и подтвердите `yes` в течение 150 секунд после запроса.
Независимый таймер отката запускается на 180 секунд перед применением правил;
запоздалое подтверждение после его срабатывания не принимается.
...
Если при установке выбран другой порт API, замените 2222 его значением. Пробы с
